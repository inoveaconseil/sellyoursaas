...
if [[ -x /usr/bin/zstd && "x$usecompressformatforarchive" == "xzstd" ]]; then
	...
	chown root:jarvis $targetdir2/conffiles.tar.zst
	...
	chown root:jarvis $targetdir/${dbname}.sql.zst
	...
	chown root:jarvis $targetdir2/conffiles.tar.gz
	...
	chown root:jarvis $targetdir/${dbname}.sql.gz
